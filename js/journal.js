(function initMojaZahradaJournalModule(globalScope) {
  ...
  if (typeof registry.createJournalModule !== "function") {
    registry.createJournalModule = function createJournalModule(deps = {}) {
      ...
      const ENTRY_TYPES = requireValue("ENTRY_TYPES", rawEntryTypes ?? constants.JOURNAL_ENTRY_TYPES);
      ...
      function normalizeJournalEntry(entry = {}, varieties = []) {
        try {
          ...
          return normalizeEntitySyncMeta({
            ...
            images,
            entryType: (() => {
              const normalizedType = String(entry.entryType || ENTRY_TYPES.NOTE).trim() || ENTRY_TYPES.NOTE;
              return normalizedType === ENTRY_TYPES.OBSERVATION ? ENTRY_TYPES.NOTE : normalizedType;
            })(),
          ...
          reportModuleError(error, {
            operation: "normalizeJournalEntry",
        ...
        return activeSyncItems(state.journal)
          .map((entry) => {
            try {
              ...
              reportModuleError(error, {
                operation: "normalizedJournalList",
                entryId: String(entry?.id || "").trim()
              });
              ...
              return {
                ...
                entryType: String(entry?.entryType || ENTRY_TYPES.NOTE).trim() || ENTRY_TYPES.NOTE,
